async function getCP150ECG(data)
{
    ...
    const gruposUnicosOrdenados = [...new Set(cp150Data.map(item => item.Group))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = cp150Data.find(item => item.Group === a)?.Order_Group || 0;
        const orderB = cp150Data.find(item => item.Group === b)?.Order_Group || 0;
        return orderA - orderB;
    });

    const familiasUnicasOrdenadas = [...new Set(cp150Data.map(item => item.Family))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = cp150Data.find(item => item.Family === a)?.Order_Family || 0;
        const orderB = cp150Data.find(item => item.Family === b)?.Order_Family || 0;
        return orderA - orderB;
    });
    ...
    for(var g=0; g<gruposUnicosOrdenados.length; g++)
    {
        ...
        layout.push("\n");
        layout.push({ text: "CP150 ECG", style: 'header3', alignment: "left" });
        layout.push("\n");
        ...
        for(var f=0; f<familiasUnicasOrdenadas.length; f++)
        {
            ...
            {
                const dataFamily = cp150Data.filter(objeto => objeto.Family === familiasUnicasOrdenadas[f]);
    ...
    var cp150ECG = [
        { image: "v2/images/CP150.png", width: 620, height: 840, alignment: 'center'},
