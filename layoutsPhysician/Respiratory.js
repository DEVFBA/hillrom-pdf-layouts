async function getRespiratory(data)
{
    var layout = []

    const respiratoryData = data.filter(objeto => objeto.Category === "Respiratory");

    const gruposUnicosOrdenados = [...new Set(respiratoryData.map(item => item.Group))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = respiratoryData.find(item => item.Group === a)?.Order_Group || 0;
        const orderB = respiratoryData.find(item => item.Group === b)?.Order_Group || 0;
        return orderA - orderB;
    });

    const familiasUnicasOrdenadas = [...new Set(respiratoryData.map(item => item.Family))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = respiratoryData.find(item => item.Family === a)?.Order_Family || 0;
        const orderB = respiratoryData.find(item => item.Family === b)?.Order_Family || 0;
        return orderA - orderB;
    });
    ...
    for(var g=0; g<gruposUnicosOrdenados.length; g++)
    {
        ...
        const dataGroup = respiratoryData.filter(objeto => objeto.Group === gruposUnicosOrdenados[g]);
        ...
        layout.push("\n");
        layout.push({ text: "Respiratory", style: 'header3', alignment: "left" });
        layout.push("\n");
        ...
        for(var f=0; f<familiasUnicasOrdenadas.length; f++)
        {
            ...
            {
                const dataFamily = respiratoryData.filter(objeto => objeto.Family === familiasUnicasOrdenadas[f]);
    ...
    var respiratory = [
        { image: "v2/images/Portada-Respiratory.png", width: 620, height: 840, alignment: 'center'},
        layout,
    ]
