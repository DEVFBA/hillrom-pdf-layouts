async function getPCBasedECG(data)
{
    ...
    const gruposUnicosOrdenados = [...new Set(pcBasedData.map(item => item.Group))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = pcBasedData.find(item => item.Group === a)?.Order_Group || 0;
        const orderB = pcBasedData.find(item => item.Group === b)?.Order_Group || 0;
        return orderA - orderB;
    });

    const familiasUnicasOrdenadas = [...new Set(pcBasedData.map(item => item.Family))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = pcBasedData.find(item => item.Family === a)?.Order_Family || 0;
        const orderB = pcBasedData.find(item => item.Family === b)?.Order_Family || 0;
        return orderA - orderB;
    });
    ...
    for(var g=0; g<gruposUnicosOrdenados.length; g++)
    {
        ...
        const dataGroup = pcBasedData.filter(objeto => objeto.Group === gruposUnicosOrdenados[g]);
        ...
        layout.push("\n");
        layout.push({ text: "PC-Based ECG", style: 'header3', alignment: "left" });
        layout.push("\n");
        ...
        for(var f=0; f<familiasUnicasOrdenadas.length; f++)
        {
            ...
            {
                const dataFamily = pcBasedData.filter(objeto => objeto.Family === familiasUnicasOrdenadas[f]);
    ...
    var pcBasedECG = [
        { image: "v2/images/PCBASED.png", width: 620, height: 840, alignment: 'center'},
        layout,
    ]
