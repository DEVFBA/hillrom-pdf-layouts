async function getProBP2000(data)
{
    ...
    const gruposUnicosOrdenados = [...new Set(proBPData.map(item => item.Group))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = proBPData.find(item => item.Group === a)?.Order_Group || 0;
        const orderB = proBPData.find(item => item.Group === b)?.Order_Group || 0;
        return orderA - orderB;
    });

    const familiasUnicasOrdenadas = [...new Set(proBPData.map(item => item.Family))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = proBPData.find(item => item.Family === a)?.Order_Family || 0;
        const orderB = proBPData.find(item => item.Family === b)?.Order_Family || 0;
        return orderA - orderB;
    });
    ...
    for(var g=0; g<gruposUnicosOrdenados.length; g++)
    {
        ...
        layout.push("\n");
        layout.push({ text: "ProBP 2000", style: 'header3', alignment: "left" });
        layout.push("\n");
        ...
        for(var f=0; f<familiasUnicasOrdenadas.length; f++)
        {
            ...
            {
                const dataFamily = proBPData.filter(objeto => objeto.Family === familiasUnicasOrdenadas[f]);
    ...
    var proBP2000Layout = [
        { image: "v2/images/ProBP200.png", width: 620, height: 840, alignment: 'center'},
