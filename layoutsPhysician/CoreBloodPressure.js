async function getCoreBloodPressure(data)
{

    var layout = []
    
    const coreBloodData = data.filter(objeto => objeto.Category === "Core Blood Pressure");

    const gruposUnicosOrdenados = [...new Set(coreBloodData.map(item => item.Group))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = coreBloodData.find(item => item.Group === a)?.Order_Group || 0;
        const orderB = coreBloodData.find(item => item.Group === b)?.Order_Group || 0;
        return orderA - orderB;
    });

    const familiasUnicasOrdenadas = [...new Set(coreBloodData.map(item => item.Family))]
    .filter(Boolean) // Eliminar valores nulos o undefined
    .sort((a, b) => {
        // Ordenar por el atributo Order_Group
        const orderA = coreBloodData.find(item => item.Family === a)?.Order_Family || 0;
        const orderB = coreBloodData.find(item => item.Family === b)?.Order_Family || 0;
        return orderA - orderB;
    });
    ...
    for(var g=0; g<gruposUnicosOrdenados.length; g++)
    {
        ...
        layout.push("\n");
        layout.push({ text: "Core Blood Pressure", style: 'header3', alignment: "left" });
        layout.push("\n");
        ...
        for(var f=0; f<familiasUnicasOrdenadas.length; f++)
        {
            ...
            {
                const dataFamily = coreBloodData.filter(objeto => objeto.Family === familiasUnicasOrdenadas[f]);
    ...
    var coreBloodPressure = [
        { image: "v2/images/Core-Blood.png", width: 620, height: 840, alignment: 'center'},
...
module.exports = {
    getCoreBloodPressure : getCoreBloodPressure,
}