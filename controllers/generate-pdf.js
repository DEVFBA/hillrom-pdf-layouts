...
const getThermsOfDistribution = require('../layoutsPhysician/ThermsOfDistribution')
...
async function createPdf(params){
    try{
        ...
        let pdf = await pool.request()
            ...
            //     var printTermsConditions = params.printTermsConditions;
            //     try {
            ...
            // }
                ...
                const TyC = await getThermsOfDistribution(); 
                var idZoneType = pdf.recordsets[0][0].Id_Zone_Type;
                var priceList = params.priceList;
                var pdfVersion = params.pdfVersion;
                var printTermsConditions = params.printTermsConditions;
                try {
                    ...
                    if (printTermsConditions) {
                        request = request.input('pvTermsConditions', sql.VarChar, TyC);
                    }
                    let listPrice = await request.execute('spPDF_Prices_List_Get_Layouts');
                    ...
                    if (listPrice.recordsets[0].length === 0) {
                        ...
                        // Obtener las rutas de los parámetros generales
                        var ruta = await dbGeneralParameters.getGeneralParametersbyID("55");
                        var rutaPdf = ruta[0][0].Value;
    ...
    try{
        ...
        if(params.modality === "Master")
        {
            ...
            if(pdf.recordsets[0].length === 0 && pdf.recordsets[1].length === 0)
            {
                ...
                const zonaNombre = ""  
                const pdfVersion = "" 
                physician.createDocument(pdf.recordsets[0], pdf.recordsets[1], rutaPdf, rutaPdfIp, nombreArchivo, zonaNombre, pdfVersion);
