    ...
    if (params.modality === "Master") {
      try {
        ...
        if (pdf.recordsets[0].length === 0 && pdf.recordsets[1].length === 0) {
          ...
          var responsePdf = await physician.createDocument(
            pdf.recordsets[0],
            pdf.recordsets[1],
            rutaPdf,
            rutaPdfIp,
            ...
            try {
              ...
              response = {
                data: {
                  status: 0,
                  message:
                    "Pdf generado con exito, pero no se pudo actualizar el folio.",
                },
              };
              parentPort.postMessage(response);
            }
        ...
        response = {
          data: {
            status: 0,
            message:
              "No se pudo obtener la informacion de la base de datos para la generacion del PDF.",
          },
        };
        parentPort.postMessage(response);
      }
        ...
        if (pdf.recordsets[0].length === 0) {
          ...
          for (var i = 0; i < listaCustomerIdsUnicos.length; i++) {
            try {
              ...
              var data = pdf.recordsets[0].filter(
                (objeto) => objeto["Customer_Id"] === listaCustomerIdsUnicos[i]
              );

              var cluster = data[0].Cluster.replace(/ /g, "_")
                .normalize("NFD")
                .replace(/[\u0300-\u036f]/g, "");
              var distributor = data[0].Distributor_Name.replace(/ /g, "_")
                .normalize("NFD")
                .replace(/[\u0300-\u036f]/g, "");

              var nombreArchivo =
                params.modality + "_" + cluster + "_" + distributor;
              ...
              await physician.createDocument(
                discounts,
                data,
                rutaPdf,
                rutaPdfIp,
          ...
          setTimeout(() => {
            ...
            const salida = fs.createWriteStream(
              carpetaAComprimir + nombreArchivoZip
            );
            ...
            salida.on("close", () => {
              console.log(
                `Carpeta comprimida correctamente. Tamaño del archivo: ${archive.pointer()} bytes.`
              );
            });
          ...
          try {
            ...
            response = {
              data: {
                status: 1,
                message:
                  "Pdfs generados con exito, pero no se pudo actualizar el folio",
              },
            };
            parentPort.postMessage(response);
          }
      ...
      try {
        ...
        try {
          ...
          if (pdf.recordsets[0].length === 0) {
            ...
            var cluster = pdf.recordsets[0][0].Cluster.replace(/ /g, "_")
              .normalize("NFD")
              .replace(/[\u0300-\u036f]/g, "");
            var distributor = pdf.recordsets[0][0].Distributor_Name.replace(
              / /g,
              "_"
            )
              .normalize("NFD")
              ...
              try {
                ...
                response = {
                  data: {
                    status: 0,
                    message:
                      "PDF Generado con exito, pero no se pudo actualizar su folio en la base de datos.",
                  },
                };
                parentPort.postMessage(response);
              }
